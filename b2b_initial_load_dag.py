...
@dag(
    ...
    doc_md="""
    ### B2B Sales Initial Load DAG (Quoted Names & SK Logging)

    Performs the initial population of the OLAP database from the OLTP source.
    **TRUNCATES target tables (with CASCADE). Uses quoted identifiers for table names.**
    ...
    **WARNING:** Running this DAG will WIPE existing data in the target OLAP tables. Ensure OLTP has `updated_at` columns and OLAP schema is correct (SCD2 cols for DimProduct, ProductSK as PK; OpportunityID in Fact).
    """
)
def b2b_initial_load_dag():
    ...
    def _truncate_table(table_name: str, cascade: bool = False):
        ...
        # Use double quotes around table name to handle potential case sensitivity
    ...
    # Pass table names as defined in schema (mixed case)
    ...
    # Use quoted table names in insert_rows calls as well
    @task
    def load_dim_date():
        ...
        while True:
            ...
            for row in source_data:
                try:
                    ...
                    # Corrected lookup for ProductSK
                    product_sk_res = hook_olap.get_first('SELECT "ProductSK" FROM "DimProduct" WHERE "ProductID" = %s AND "IsCurrent" = TRUE', parameters=(oltp_product_id,))
                    ...
                    product_key = product_sk_res[0] if product_sk_res else None # This is the ProductSK
                    ...
                    if all([date_key, account_key, product_key, agent_key, stage_key]):
                         # Include OpportunityID if needed for UPSERT later, use ProductSK for ProductKey
                         # Assuming OpportunityID column exists now based on previous recommendations
